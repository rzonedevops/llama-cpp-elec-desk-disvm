...
infer(req: ref InferenceRequest): ref InferenceResponse
{
	...
	# For production with actual llama.cpp inference, use llambo-ffi module:
	#
	#   include "llambo-ffi.m";
	#   ffi := load LlamboFFI LlamboFFI->PATH;
	#   bridge := Bridge.connect("");
	#   (ok, msg, result) := bridge.infer(req.prompt);
	#
	# See FFI-README.md for complete integration instructions.
